...
int main(int argc, char *argv[]){
	...
	do {
                int n;
                char file_name[MAX_FILE_NAME] = "./data/620.jpg";
                char text_file[MAX_FILE_NAME] = "./data/test.txt";
                char extension[EXT_SIZE] = "jpg";
                int size_of_file = 0;
                int size_of_text_file = 0;
                char *buffer     = NULL;
                char *new_buffer = NULL;

                // printf("****************** WELCOME TO IMAGE TRANSFER WITHOUT FTP 2 *****************\n");
...
//Comment
		...
		// if (n < size_of_file) {
                ...
                // Send text file
                n = 0;
                if(read_image_file(text_file, &new_buffer, &size_of_text_file)){
                        error("\t Couldn't read text file", READ_FAIL);
                        goto safe_exit;
                }
                n = send_to_server(sockfd, &size_of_text_file, sizeof(int));	//Write to server
		if (n < 0) {
                        goto safe_exit;
                }
		// n = send_to_server(sockfd, extension, EXT_SIZE);	//Write to server
		...
		if (n < size_of_text_file) {
                ...
                n = send_to_server(sockfd, new_buffer, size_of_text_file);
	} while(0);
