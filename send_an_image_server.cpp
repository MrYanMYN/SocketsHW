...
TODO:  
 ...
 - Add text sending for the values of X Y H W {Done}
 - Add parsing for text Document
 - remove the need to enter manually the ip address and port {Half done}
 - Integrate the code with opencv2
 - check if the side can send the cropeed pic and X Y H W
 - Make the program work with a stream of data that is continues   
...
//-------------------------------------------------------
                ...
                buffer    = NULL;
                buffer_size = 0;
                char extension_two[4];
                char file_name_two[256];
                fp = NULL;
                ...
                strcpy(file_name_two, "incoming-text");
                strcat(file_name_two, ".txt");
                fp = fopen(file_name_two, "wb");
                if (fp == NULL) {
                    error("Error opening file");
                }
                fwrite(buffer, buffer_size, 1, fp);
